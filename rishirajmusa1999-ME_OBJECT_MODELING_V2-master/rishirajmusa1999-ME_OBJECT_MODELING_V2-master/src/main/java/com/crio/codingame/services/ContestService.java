...
public class ContestService implements IContestService {
    ...
    List<Question> pickQwithnum = questions.stream().collect(Collectors.toList());
    int size = pickQwithnum.size();
    Random rand = new Random();
    if(size<=2){
        return questions;
    }
    final List<Question> newListPick = new ArrayList<>();
    for (int i=0;i<numQuestion;i++){
        int randomIndex = rand.nextInt(pickQwithnum.size());
        newListPick.add(pickQwithnum.get(randomIndex));
        pickQwithnum.remove(randomIndex);
    }
        return newListPick;
    }
